...
def build_dataset(manifest_rows, args, feature_order):
    ...
    for i, row in enumerate(manifest_rows, start=1):
        ...
        avg = elapsed / i
        eta = avg * (n - i)
        print(f"  [{i}/{n}] {anon_id}  ({avg:.1f}s/call, ETA {eta/60:.1f} min)", flush=True)
...
def main():
    ...
    if args.limit:
        def stratified_sample(split_rows, limit):
            per_class = max(limit // 2, 1)
            human = [r for r in split_rows if r["label"] == "human"][:per_class]
            synthetic = [r for r in split_rows if r["label"] == "synthetic"][:per_class]
            return human + synthetic
        train_rows = stratified_sample(train_rows, args.limit)
        val_rows = stratified_sample(val_rows, args.limit)
